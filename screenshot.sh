...
# 使用 BDInfo 生成详细蓝光光盘信息
echo -e "\n${bold}Generating detailed Blu-ray info with BDInfo...${normal}"
if command -v bdinfo &>/dev/null; then
    # 找到蓝光光盘根目录（BDMV的父目录）
    bd_root=$(dirname "$bdmv_dir")

    echo -ne "Running ${blue}bdinfo${normal} on ${yellow}$bd_root${normal} ...  "

    # 创建临时文件存储 bdinfo 输出
    bdinfo_temp="${outputpath}/${file_title_clean}.bdinfo.tmp.txt"

    # 运行 bdinfo 命令，输出到临时文件
    if bdinfo "$bd_root" "$outputpath" > "$bdinfo_temp" 2>&1; then
        echo -e "${green}DONE${normal}"

        # 将 bdinfo 报告添加到 mediainfo 文件
        if ls "${outputpath}/"*.bdinfo.txt 1>/dev/null 2>&1; then
            echo -ne "Adding BDInfo report to ${blue}${file_title_clean}.mediainfo.txt${normal} ...  "
            echo -e "\n\n========================================" >> "${outputpath}/${file_title_clean}.mediainfo.txt"
            echo -e "BDInfo Detailed Report" >> "${outputpath}/${file_title_clean}.mediainfo.txt"
            echo -e "========================================\n" >> "${outputpath}/${file_title_clean}.mediainfo.txt"

            # 添加所有生成的 bdinfo 报告
            for bdinfo_file in "${outputpath}/"*.bdinfo.txt; do
                cat "$bdinfo_file" >> "${outputpath}/${file_title_clean}.mediainfo.txt"
                echo -e "\n" >> "${outputpath}/${file_title_clean}.mediainfo.txt"
            done
            echo -e "${green}DONE${normal}"
        else
            echo -e "${yellow}Warning: No BDInfo report generated${normal}"
        fi
    else
        echo -e "${red}FAILED${normal}"
        echo -e "${yellow}BDInfo output:${normal}"
        cat "$bdinfo_temp"
    fi
    ...
    echo -e "${yellow}BDInfo tool not found, skipping detailed Blu-ray analysis${normal}"
    echo -e "${yellow}Tip: Install bdinfo for comprehensive Blu-ray disc information${normal}"
fi ; }
